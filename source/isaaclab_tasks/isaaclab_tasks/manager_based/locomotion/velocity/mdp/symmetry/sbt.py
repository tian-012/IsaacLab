...
def _transform_policy_obs_left_right(env: ManagerBasedRLEnv, obs: torch.Tensor) -> torch.Tensor:
    ...
    # joint pos
    obs[:, 9:21] =  _switch_sbt_joints_left_right(obs[:, 9:21])
    # joint vel
    obs[:, 21:33] = _switch_sbt_joints_left_right(obs[:, 21:33])
    # last actions
    obs[:, 33:45] = _switch_sbt_joints_left_right(obs[:, 33:45])
...
def _transform_actions_left_right(actions: torch.Tensor) -> torch.Tensor:
    ...
    actions[:] = _switch_sbt_joints_left_right(actions[:])
    return actions
...
def _switch_sbt_joints_left_right(joint_data: torch.Tensor) -> torch.Tensor:
    ...
    joint_data_switched[..., [0, 1, 2, 3, 10, 11]] *= -1.0
